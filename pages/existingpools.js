...
export const getExistingPools = async () => {
  ...
  try {
    ...
    const newPool = Object.keys(callResults)
      .map((key) => callResults[key])
      .map((e) => e.callsReturnContext[0].returnValues[0]);
    return newPool;
  } catch (err) {
    console.log(err);
  }
...
const ExistingPools = ({ connectedAddress }) => {
  ...
  const handleGetWalletAndPoolBalance = async (
    nftCollection,
    walletAddress,
    poolAddress
  ) => {
    ...
    try {
      ...
      const poolTokenWalletBalance = await getPoolTokenBalance(
        poolAddress,
        walletAddress
      );
  ...
  const handleDepositNFT = async (pool) => {
    try {
      const nftID = depositInputText.current.value
        .split(",")
        .map((e) => {
          const num = Number(e);
  ...
  const handleWithdrawNFT = async (pool) => {
    try {
      const nftID = withdrawInputText.current.value
        .split(",")
        .map((e) => {
          const num = Number(e);
          return Number.isInteger(num) ? num : "";
      ...
      if (err.message === "Internal JSON-RPC error.") {
        alert(
          "Metamask Error: Please make sure you have enough pool token/enter the correct NFT id (check nft id available in the pool) and try again"
        );
      }
    }
  };
  ...
  const handleSwapNFT = async (pool) => {
    ...
    const swapOutIds = swapOutInputText.current.value
      .split(",")
      .map((e) => {
        const num = Number(e);
        return Number.isInteger(num) ? num : "";
      ...
      .filter((e) => e !== "");
    const swapInIds = swapInInputText.current.value
      .split(",")
      .map((e) => {
        const num = Number(e);
        return Number.isInteger(num) ? num : "";
    ...
    try {
      ...
      const nftSwapContract = new ethers.Contract(
        poolContract.swapContract.address,
        poolContract.swapContract.abi,
        signer
      );

      const tx = await nftSwapContract.swapTokens(
        collectionAddress,
        swapOutIds,
        swapInIds
      );
      ...
      if (err.message === "Internal JSON-RPC error.") {
        alert(
          "Metamask Error: Please make sure you input the correct NFT ids and try again"
        );
      }
    }
  };
  ...
  const displayNFTGrid = (pool, type) => {
    if (!pool.isUpdated) {
      return <div>Click load to get NFTs</div>;
    }

    const Wallet_grid = ({ pool, type = "deposit" }) => {
      return pool.walletTokens.map((token, index) => (
        <div
          onClick={() => handleAddSelected(token.id, type)}
          key={`${pool.uuid}-${token.id}`}
      ...
      return (
        <div>
          {pool.nftTokenWalletBalance > 0 ? (
            <h3>
              {`${pool.nftTokenWalletBalance} ${pool.collectionName}`} you have
            </h3>
          ) : (
            <h3>You don&apos;t have any {`${pool.collectionName}`}</h3>
          )}
          ...
          {pool?.holdingsLength > 0 ? (
            <h3>
              {`${pool.holdingsLength} ${pool.collectionName}`} in the pool
            </h3>
          ) : (
            <h3>This pool don&apos;t have any {`${pool.collectionName}`}</h3>
          )}
      ...
      <div className="py-2 px-4 bg-gray-200">
        <div className="flex justify-between">
          ...
          <p className="text-3xl py-2">
            You have {Number(pool.nftTokenWalletBalance)} {pool.collectionName}{" "}
          </p>
        </div>
        {pool?.nftTokenWalletBalance >= 0 ? (
          <div>
            <div>
                ...
                <input
                  ...
                  placeholder={
                    pool.isDepositApproved
                      ? `Enter NFT ID (use comma to separate multiple IDs)`
                      : "Not yet approve deposit"
                  }
                  className={`w-full px-4 py-2 text-gray-700 bg-white border border-gray-400 rounded-lg shadow-sm focus:outline-none focus:border-blue-500 ${
                    pool.isDepositApproved
                      ? "bg-white"
                      : "bg-gray-500 pointer-events-none"
                  }`}
                />
                <button className={`${buttonStyle}`} onClick={handleInputClear}>
                  Clear
                </button>
                <button
                  className={`${buttonStyle}`}
                  onClick={() => handleDepositNFT(pool)}
                >
                  Deposit
                </button>
                <button
                  className={`${buttonStyle}`}
                  onClick={() => handleCashout(pool)}
                >
                  Cashout
                </button>
              </div>
              ...
              <div className="flex justify-between my-2">
                {isLoading ? (
                  <FaSync className="animate-spin text-black text-3xl" />
                ) : (
                  <button
                    className={`${buttonStyle} w-2/5`}
                    onClick={() =>
                      handleGetWalletAndPoolBalance(
                        pool.collectionAddress,
                        connectedAddress,
                        pool.poolAddress
                      )
                ...
                {pool.isDepositApproved ? (
                  ...
                  <button
                    className={`${buttonStyle} w-2/5`}
                    onClick={() => handleApproveDeposit(pool)}
                  >
                    Approve Deposit
                  </button>
                )}
      ...
      <div className="py-2 px-4 bg-gray-200">
        <div className="flex justify-between">
          ...
          <p className="text-xl py-2">
            Wallet Balance: {Number(pool.poolTokenWalletBalance).toFixed(4)}{" "}
            {pool.symbol} <br /> (can withdraw{" "}
            {Math.floor(Number(pool.poolTokenWalletBalance))}{" "}
            {pool.collectionName}){" "}
          </p>
        </div>
          ...
          <div>
            <div>
              <div className="flex flex-row items-center space-x-4">
                ...
                <button
                  className={`${buttonStyle}`}
                  onClick={() => handleWithdrawNFT(pool)}
                >
                  Withdraw
                </button>
              </div>
              ...
              <div className="flex justify-between my-2">
                {isLoading ? (
                  ...
                  <button
                    className={`${buttonStyle} w-2/5 my-2`}
                    onClick={() =>
                      handleGetWalletAndPoolBalance(
                        pool.collectionAddress,
                        connectedAddress,
                  ...
                  >
                    {pool.poolTokens.length === 0 ? "Get" : "Refresh"}{" "}
                    {pool.collectionName} pool
                  </button>
                )}
                ...
                {pool.poolTokenApproved ? (
                  ...
                  <button
                    className={`${buttonStyle} w-2/5`}
                    onClick={() => handleApprovePoolToken(pool)}
                  >
                    Approve Token
                  </button>
                )}
        ...
        <div>
          <div className="flex flex-row items-center space-x-4 my-5">
            ...
            <input
              ...
              placeholder={
                pool.isSwapApproved
                  ? "Enter the ID you wish to send out"
                  : "Not yet approve swap"
              }
              className={`w-full px-4 py-2 text-gray-700 bg-white border border-gray-400 rounded-lg shadow-sm focus:outline-none focus:border-blue-500 ${
                pool.isSwapApproved
                  ? "bg-white"
          ...
          <div className="flex justify-between my-2">
            ...
            {pool.isSwapApproved ? (
              ...
              <button
                className={`${buttonStyle} w-2/5`}
                onClick={() => handleApproveSwap(pool)}
              >
                Approve Swap
              </button>
            )}
  ...
  const RenderPools = ({ poolsDetail }) => {
    return poolsDetail.map((pool) => {
      return (
        <div key={pool.uuid}>
          <div
            className={`flex flex-col mx-auto w-full shadow-lg rounded-lg my-4 ${
              userSelected?.id === pool.uuid ? "bg-gray-300" : "bg-white"
            }`}
          >
            <div className="px-8 py-4 flex flex-col items-center sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="text-center sm:text-left sm:pl-4">
                <a
                  ...
                  rel="noopener noreferrer"
                >
                  <h1 className="text-2xl font-bold text-gray-800 hover:text-blue-500">
                    {pool.name}{" "}
                  </h1>
                </a>
                <div className="flex flex-col items-center sm:flex-row sm:justify-start mt-2">
                  <a
                    href={`https://opensea.io/assets?search[query]=${pool.collectionAddress}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mr-0 sm:mr-4"
                  >
                    <p className="text-md font-bold text-gray-500 hover:text-blue-500">
                      Collection: {pool.collectionName}
                    </p>
                  </a>
                </div>
              </div>
            ...
            {userSelected?.id === pool.uuid ? (
              <div className="">
                <div className="px-8 py-4 flex my-2 justify-between">
                  <button
                    className={`${buttonStyle} w-1/4 h-full`}
                    onClick={() => handleDepositSection(pool)}
                  >
                    {" "}
                    Deposit (Wallet: {pool.nftTokenWalletBalance}){" "}
                  </button>
                  <button
                    className={`${buttonStyle} w-1/4 h-full`}
                    onClick={() => handleWithdrawSection(pool)}
                  >
                    {" "}
                    Withdraw (Pool: {pool.holdingsLength}){" "}
                  </button>
                  <button
                    className={`${buttonStyle} w-1/4 h-full`}
                    onClick={() => handleSwapSection(pool)}
                  >
                    {" "}
                    Swap{" "}
                  </button>
                </div>
                {userSelected?.depositClicked ? (
                  <DepositInput pool={pool} />
                ) : null}
                {userSelected?.withdrawClicked ? (
                  <WithdrawInput pool={pool} />
                ) : null}
        ...
        <div className="mx-auto w-90/100 bg-white shadow-lg rounded-lg p-4 ">
          ...
          <div>
            {userSelected?.activePool ? (
              <button className="underline" onClick={handleGoBackPools}>
                {" "}
                ← Back{" "}
              </button>
            ) : (
              <div className="underline">Existing Collections ↓</div>
            )}
          </div>
          <div className="overflow-y-scroll py-2 max-h-[85vh]">
            {poolsDetail?.length > 0 ? (
              userSelected?.activePool ? (
                <RenderPools
                  poolsDetail={poolsDetail.filter(
                    (pool) => pool.poolAddress === userSelected.activePool
                  )}
                />
              ) : (
